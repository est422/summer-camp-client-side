    ...
    <header
      className={`fixed left-0 top-0 w-full z-99999 py-7 ${
        stickyMenu
          ...
          : "bg-meta text-blackho dark:bg-black dark:text-meta"
      }`}
    >
          ...
          <nav>
            <ul className="flex lg:items-center flex-col lg:flex-row gap-5 lg:gap-10">
              {menuData.map((menuItem, key) => {
                if (!userInfo && menuItem.title === "Profile") {
                  return null;
                }

                if (menuItem.title != "Sign In" && menuItem.title != "Sign Up")
                  return (
                    <li
                      key={key}
