...
import { volunteersInfo } from "@/types/volunteersInfo";

interface VolunteersApplicationFormData {
  imageName: string;
  fileName: string;
  fname: string;
  lname: string;
...
const VolunteersApplicationForm = () => {
  const [images, setImages] = useState<File[]>([]);
  const [cv, setCv] = useState<File[]>([]);
  const [formData, setFormData] = useState<VolunteersApplicationFormData>({
    imageName: "",
    fileName: "",
    fname: "",
    lname: "",
  ...
  const [volunteersInfo, setVolunteersInfo] = useState<volunteersInfo>();
            ...
            <div className="flex w-full border-l-6 border-[#34D399] bg-[#34D399] bg-opacity-[15%] px-7 py-8 shadow-md dark:bg-[#1B1B24] dark:bg-opacity-30 md:p-9">
              <div className="mr-5 flex h-9 w-full max-w-[36px] items-center justify-center rounded-lg bg-[#34D399]">
                <button
                  onClick={(e) => {
                    e.preventDefault();
                    // setSuccessFull("");
                    router.push(`/volunteers/details/${volunteersInfo?.id}`);
                  }}
                >
                  <svg
                    width="16"
              ...
              <div className="w-full">
                <h5 className="mb-3 text-lg font-semibold text-black dark:text-[#34D399] ">
                  Volunteer application has been Sent Successfully
                </h5>
              </div>
            </div>
          </div>
...
export default VolunteersApplicationForm;
